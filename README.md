...
- **Calendar export** – choose any writable calendar, then export one or many workouts with a single tap. Once you've picked a calendar it becomes the default for future exports until you change it.
- **Flexible history** – adjust how far back the workout list reaches, from the past few months to your entire Health history.

## Usage tips

1. Launch the app and approve the Health access prompt. Recent workouts from the history window you selected in **Settings** will appear automatically.
2. If prompted, grant Calendar access so the app can filter out workouts that already exist in your calendars.
3. Tap **Select All** or pick individual workouts, then tap **Export**. The app uses your saved default calendar automatically; adjust it in **Settings** whenever you need to switch destinations.

Open the **Settings** panel from the gear icon to tweak the workout lookback window or change the default calendar at any time.
